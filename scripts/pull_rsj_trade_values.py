#!/usr/bin/env python
"""Pulls Roto Street Journal's trade value chart (QB/RB/WR/TE) for the
current week and writes them to data/raw/rsj_trade_values/ (immutable
...
Usage:
    python scripts/pull_rsj_trade_values.py               # current week, all 4 positions
    python scripts/pull_rsj_trade_values.py --week 2       # force a specific week
    python scripts/pull_rsj_trade_values.py --position rb,wr

A position not yet published for the target week is skipped (not an error --
see src/sources/rsj_trade_values.py; RSJ publishes 4 separate position
pages, staggered like Boone's). Exits non-zero only on a validation error or
unexpected failure. Writes data/last_rsj_trade_values_status.json (position
-> ok|failed: <reason>|not_yet_published) so an external check
...
from src.sources.rsj_trade_values import (
    POSITIONS,
    RsjTradeValueFetchError,
...
STATUS_PATH = BASE_DIR / "data" / "last_rsj_trade_values_status.json"
...
def main():
    ...
    print(f"Starting RSJ trade-value pull -- week {week}, positions {wanted_positions}")
    print(f"   {start.strftime('%H:%M:%S')}\n")

    try:
        urls = discover_position_urls(week)
    except RsjTradeValueFetchError as e:
    ...
    for i, position in enumerate(wanted_positions, 1):
        ...
        rows = normalize.rsj_trade_values_to_rows(
        ...
        snapshot_path = storage.trade_value_raw_snapshot_path(
            RAW_DIR / "rsj_trade_values", "rsj", args.season, week, position, pulled_at
        )
