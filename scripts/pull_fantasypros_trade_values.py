#!/usr/bin/env python
"""Pulls FantasyPros' trade value chart (QB/RB/WR/TE) for the current week
and writes them to data/raw/fantasypros_trade_values/ (immutable snapshot)
and data/processed/trade_values_long.csv (append) -- the same shared table
every trade-value source appends to, distinguished by source="fantasypros".
...
Usage:
    python scripts/pull_fantasypros_trade_values.py               # current week, all 4 positions
    python scripts/pull_fantasypros_trade_values.py --week 2       # force a specific week
    python scripts/pull_fantasypros_trade_values.py --position rb,wr

FantasyPros publishes one combined article per week (not one per position),
so either the whole pull succeeds or none of it does -- there's no
per-position "not yet published" state the way Boone/RSJ have. Exits
non-zero only on a validation error or unexpected failure. Writes
data/last_fantasypros_trade_values_status.json (position -> ok|failed:
<reason>|not_yet_published) so an external check (scheduled_pull.ps1) can
...
from src.sources.fantasypros_trade_values import (
    POSITIONS,
    FantasyProsTradeValueFetchError,
...
STATUS_PATH = BASE_DIR / "data" / "last_fantasypros_trade_values_status.json"
...
def main():
    ...
    print(f"Starting FantasyPros trade-value pull -- week {week}, positions {wanted_positions}")
    print(f"   {start.strftime('%H:%M:%S')}\n")

    try:
        url = discover_article_url(week)
    except FantasyProsTradeValueFetchError as e:
    ...
    for i, position in enumerate(wanted_positions, 1):
        ...
        rows = normalize.fantasypros_trade_values_to_rows(
        ...
        snapshot_path = storage.trade_value_raw_snapshot_path(
            RAW_DIR / "fantasypros_trade_values", "fantasypros", args.season, week, position, pulled_at
        )
