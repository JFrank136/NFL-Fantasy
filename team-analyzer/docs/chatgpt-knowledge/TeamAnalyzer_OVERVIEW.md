...
Team Analyzer is a browser-based fantasy football trade-value tool Jared built last season (a "v1 core," per its own README). It lives at `in-season/team-analyzer/` — it was moved there recently as part of consolidating his fantasy football tools under one `in-season/` folder, but it's a self-contained project with its own history, not something built alongside the new in-season pipeline. It hasn't been updated for the current season yet and is expected to need rework before it's used again.
...
Separately, there's a `scrapers/` folder with standalone Python scripts that were used to *pull* the Boone and DraftSharks CSVs automatically (browser automation for Boone, a more direct scrape for DraftSharks) instead of manually exporting them. These are decoupled from the web app — they just produce the CSV files a user would otherwise upload by hand.

## Status relative to the new in-season pipeline

This predates and duplicates part of what the new `in-season/` weekly-rankings pipeline (see the main in-season knowledge files) now does more robustly — that pipeline already pulls Boone and DraftSharks weekly rankings on a schedule with validation. Team Analyzer's own scrapers were a rougher, earlier version of that same idea. Any future rework of Team Analyzer should probably consume the new pipeline's data rather than keep its own separate scraping path — but that decision hasn't been made yet; for now this file just documents what exists.
